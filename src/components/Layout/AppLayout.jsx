...
export function AppLayout() {
    ...
    // Notifications system imports
    const showConfirm = useStore(state => state.showConfirm);
    const notifications = useStore(state => state.notifications) || [];
    const markNotificationAsRead = useStore(state => state.markNotificationAsRead);
    const clearNotifications = useStore(state => state.clearNotifications);
    const notiTargetId = currentUser?.role === 'staff' ? currentUser.createdBy : currentUser?.username;
    const myNotifications = notifications.filter(n => n.userId === notiTargetId);
    ...
    const closeVipExpiredModal = useStore(state => state.closeVipExpiredModal);
    const USERS = useStore(state => state.USERS);

    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const [isNotiOpen, setIsNotiOpen] = useState(false);
    const [selectedUpgradeReq, setSelectedUpgradeReq] = useState(null);
    ...
    // Kiểm tra hết hạn VIP realtime (mỗi 60 giây)
    const updateUser = useStore(state => state.updateUser);
    useEffect(() => {
        if (currentUser?.role !== 'admin' || !currentUser?.expiresAt || currentUser?.showVipExpired) return;
        const check = () => {
            const isExpired = Date.now() > new Date(currentUser.expiresAt).getTime();
            if (isExpired && currentUser.isPremium) {
                updateUser(currentUser.username, { isPremium: false, showVipExpired: true });
            }
        };
        check(); // check ngay khi mount
        const interval = setInterval(check, 60000);
        return () => clearInterval(interval);
    }, [currentUser?.username, currentUser?.expiresAt, currentUser?.showVipExpired]);
    ...
    return (
        <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[110] px-4 animate-fade-in">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 text-center relative animate-slide-up overflow-hidden">
                        ...
                        <p className="text-slate-600 mb-8 font-medium leading-relaxed">
                            Tài khoản của bạn đã được hệ thống xác nhận lên hạng VIP. Kể từ lúc này bạn có thể sử dụng các tính năng cao cấp không giới hạn thiết bị.
                        </p>
                        <button
            ...
            {currentUser?.showVipExpired && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[110] px-4 animate-fade-in">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 text-center relative animate-slide-up overflow-hidden">
                        ...
                        <h2 className="text-2xl font-extrabold text-slate-800 mb-2">Gói VIP Đã Hết Hạn</h2>
                        <h3 className="text-lg font-bold text-slate-500 mb-4">{currentUser.fullname || currentUser.username}</h3>
                        <p className="text-slate-600 mb-8 font-medium leading-relaxed">
                            Tài khoản của bạn đã hết hạn sử dụng các tính năng cao cấp. Bạn có muốn gia hạn để tiếp tục sử dụng không?
                        </p>
                        <div className="flex gap-3">
                            <button
                                onClick={() => closeVipExpiredModal(currentUser.username)}
                            ...
                            <button
                                onClick={() => {
                                    closeVipExpiredModal(currentUser.username);
                                    setUpgradeModalOpen(true);
                                }}
                                className="flex-1 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl shadow-md shadow-emerald-500/20 active:scale-95 transition-all"
                            >
                                Có, Mở Bảng Giá
                            </button>
                        </div>
            ...
            {/* Sadmin detailed VIP approval Modal */}
            {selectedUpgradeReq && currentUser?.role === 'sadmin' && (() => {
                const targetUserObj = USERS.find(u => u.username === selectedUpgradeReq.username);
                const baseDate = (targetUserObj?.expiresAt && new Date(targetUserObj.expiresAt).getTime() > Date.now())
                    ? new Date(targetUserObj.expiresAt)
                    : new Date();
                const fromDateStr = baseDate.toLocaleDateString('vi-VN');
                const toDate = new Date(baseDate.getTime());
                toDate.setDate(toDate.getDate() + (selectedUpgradeReq.months * 30));
                const toDateStr = toDate.toLocaleDateString('vi-VN');

                return (
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[110] px-4 animate-fade-in">
                        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 relative animate-slide-up overflow-hidden">
                            <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-4">
                                <h3 className="text-[17px] font-bold text-slate-800 flex items-center gap-2">
                                    <Zap className="w-5 h-5 text-amber-500 fill-amber-500" />
                                    Xem gói nâng cấp
                                </h3>
                                <button onClick={() => setSelectedUpgradeReq(null)} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-full transition-colors"><XCircle className="w-5 h-5" /></button>
                            </div>
                            <div className="space-y-3 mb-6 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-500 font-medium">Tài khoản:</span>
                                    <span className="text-sm font-bold text-slate-800">@{selectedUpgradeReq.username}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-500 font-medium">Mức gói:</span>
                                    <span className="text-sm font-bold text-amber-600">{selectedUpgradeReq.planName} ({selectedUpgradeReq.months} tháng)</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-500 font-medium">Ngày gửi n.cấp:</span>
                                    <span className="text-sm font-medium text-slate-700">{new Date(selectedUpgradeReq.time).toLocaleDateString('vi-VN')}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-500 font-medium">Ngày hết hạn:</span>
                                    <span className="text-sm font-bold text-emerald-600">{toDateStr}</span>
                                </div>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={() => setSelectedUpgradeReq(null)} className="flex-1 py-2.5 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl transition-colors">Đóng</button>
                                <button onClick={() => {
                                    showConfirm(`Bạn xác nhận cấp quyền cho tài khoản này lên vip từ ngày ${fromDateStr} đến ngày ${toDateStr} không?`, () => {
                                        approveUpgrade(selectedUpgradeReq.id);
                                        setSelectedUpgradeReq(null);
                                        showToast('Nâng cấp tài khoản thành công!');
                                    });
                                }} className="flex-1 py-2.5 px-4 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-xl transition-colors shadow-sm shadow-emerald-500/20 active:scale-95">Phê duyệt</button>
                            </div>
                        </div>
                    </div>
                )
            ...
            <main className="flex-1 w-full md:ml-24 lg:ml-64 h-full relative flex flex-col transition-all duration-300">
                ...
                <header className="md:hidden h-14 bg-white border-b border-slate-200 flex items-center px-4 justify-between sticky top-0 z-40">
                    ...
                    <div ref={headerMenuRef} className="flex items-center gap-3 relative">
                                                ...
                                                upgradeRequests.map(req => (
                                                    <div key={req.id} className="p-3 bg-slate-50 border border-slate-200 rounded-2xl">
                                                        ...
                                                        <div className="flex gap-2 mt-3">
                                                            <button onClick={() => { setSelectedUpgradeReq(req); setIsNotiOpen(false); }} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-1.5 px-3 rounded-xl text-xs flex justify-center items-center gap-1 transition-colors shadow-sm focus:ring-4 focus:ring-emerald-500/20 active:scale-95">
                                                                <CheckCircle className="w-3.5 h-3.5" /> Xem gói nâng cấp
                                                            </button>
                        ...
                        {/* Biểu tượng Chuông Thông Báo (Chỉ dành cho Khách Hàng/Cửa Hàng) */}
                        {(currentUser?.role === 'admin' || currentUser?.role === 'staff') && (
                            <div className="relative">
                                ...
                                {/* Dropdown Nhận thông báo */}
                                {isNotiOpen && (
                                    <div className="absolute top-[calc(100%+8px)] right-0 w-80 bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-4 animate-slide-up z-50 origin-top-right overflow-hidden flex flex-col max-h-[85vh]">
                                        <div className="flex items-center justify-between mb-3 border-b border-slate-100 pb-3">
                                            <h3 className="font-bold text-slate-800 flex items-center gap-2">Thông báo <span className="bg-red-100 text-red-600 text-xs px-2 py-0.5 rounded-full">{unreadCount}</span></h3>
                                            {myNotifications.length > 0 && (
                                                <button onClick={() => clearNotifications(notiTargetId)} className="text-xs text-slate-400 hover:text-red-500 transition-colors font-medium">Bỏ qua tất cả</button>
                                            )}
                                        </div>
                                        <div className="flex-1 overflow-y-auto space-y-3 pb-2 custom-scrollbar pr-1">
                                            {myNotifications.length === 0 ? (
                                                ...
                                                myNotifications.map(noti => (
                                                    <div key={noti.id} className={`p-3 rounded-2xl border transition-all ${noti.read ? 'bg-white border-slate-100 opacity-60' : 'bg-emerald-50 border-emerald-100'}`}>
                                                        <div className="flex items-start gap-3">
                                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${noti.read ? 'bg-slate-100 text-slate-500' : 'bg-emerald-200 text-emerald-700 shadow-sm'}`}>
                                                                <Bell className="w-4 h-4" />
                                                            </div>
                                                            <div className="flex-1 min-w-0">
                                                                <p className={`text-sm font-semibold truncate break-all mb-0.5 ${noti.read ? 'text-slate-600' : 'text-emerald-800'}`}>{noti.title}</p>
                                                                <p className="text-xs text-slate-600 leading-snug">{noti.message}</p>
                                                                <p className="text-[10px] text-slate-400 mt-2 font-medium">{new Date(noti.time).toLocaleString('vi-VN')}</p>
                                                            </div>
                                                        </div>
                                                        {!noti.read && (
                                                            <div className="flex gap-2 mt-3 pt-3 border-t border-emerald-100/50">
                                                                <button onClick={() => markNotificationAsRead(noti.id)} className="flex-1 bg-white hover:bg-slate-50 text-slate-700 font-bold py-1.5 px-3 rounded-xl text-xs flex justify-center items-center gap-1 transition-colors shadow-sm focus:ring-4 focus:ring-slate-100 border border-slate-200 active:scale-95">
                                                                    <CheckCircle className="w-3.5 h-3.5" /> Đánh dấu đã đọc
                                                                </button>
                                                            </div>
                                                        )}
                                                    </div>
                        ...
                        <button onClick={() => { setIsDropdownOpen(!isDropdownOpen); setIsNotiOpen(false); }} className="relative w-8 h-8 md:w-10 md:h-10 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center active:scale-95 transition-transform border border-emerald-200 shadow-sm" title="Tuỳ chọn tài khoản">
                            {currentUser?.avatar ? (
                            ...
                            <div className="absolute top-[calc(100%+8px)] right-0 w-56 bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 p-2 animate-slide-up z-50 origin-top-right">
                                <div className="p-3 mb-1 border-b border-slate-100 flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0 relative">
                                        ...
                                        {currentUser?.role === 'sadmin' && <span className="absolute -top-1.5 -right-1.5 text-base leading-none select-none">👑</span>}
                                        {currentUser?.isPremium && currentUser?.role !== 'sadmin' && <span className="absolute -top-1.5 -right-1.5 text-base leading-none select-none">💎</span>}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="font-bold text-slate-800 truncate text-sm">{currentUser?.fullname || currentUser?.username}</p>
                                        {currentUser?.role === 'sadmin' ? (
                                            <p className="text-[10px] font-bold text-violet-500 uppercase tracking-wider truncate mt-0.5">Super Admin 👑</p>
                                        ) : currentUser?.isPremium ? (
                                            <p className="text-[10px] font-bold text-amber-600 uppercase tracking-wider truncate mt-0.5">Tài khoản VIP 💎</p>
                                        ) : (
                                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider truncate mt-0.5">Gói Miễn Phí</p>
                                        )}
                                    </div>
                                </div>
